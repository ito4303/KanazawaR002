...
pref_data4 |>
  dplyr::filter(`地域` %in% c("富山県", "石川県", "福井県")) |>
  ggplot() +
  geom_area(aes(x = `年`, y = `女`), fill ="red") +
  geom_ribbon(aes(x = `年`, ymin = `女`, ymax = `総数`), fill = "blue") +
  annotate("text", x = 2000, y = 3e+5, label = "女", color = "white") +
  annotate("text", x = 2000, y = 7.2e+5, label = "男", color = "white") +
...
男女別の積み上げグラフにします。これには、`geom_col`関数の引数で`position = "stack"`とします（実はデフォルトです）。また、`scale_fill_manual(values = c("blue", "red"))`で色も指定します。
...
できたデータの`data_g`のクラスを確認しています。sf (= simple feature)クラスに属していることがわかります。
...
データに含まれるポリゴンを市町村ごとにst_union関数で結合します。また、わかりやすいように、市区町村名のフィールドのN03_004を、nameという名前に変えておきます。
...
ggplot(data) +
  geom_sf() +
  geom_sf_text(aes(label = name), size = 2.5, family = "Noto Sans JP") +
